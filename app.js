...
class ChatAIApp {
  ...
  async handleGenerateImage() {
    ...
    if (previewContainer) {
      previewContainer.innerHTML = `
        <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:320px;width:100%;background:rgba(0,0,0,0.25);border-radius:16px;padding:20px;text-align:center;">
          <p style="font-weight:700;margin-bottom:6px;font-size:1.05rem;">جاري رسم وتوليد الصورة بدقة 8K...</p>
          <p style="color:var(--text-muted);font-size:0.85rem;">يتم استخدام محرك FLUX الفائق</p>
        </div>
      `;
      // Smoothly scroll down to preview area on mobile
      ...
      if (previewContainer) {
        previewContainer.innerHTML = `
          ...
          <div style="position:absolute;bottom:16px;display:flex;gap:10px;">
            <a href="${imageUrl}" target="_blank" download="chat-ai-image.jpg" class="gradient-btn" style="padding:10px 20px;border-radius:8px;text-decoration:none;display:flex;align-items:center;gap:8px;font-size:0.95rem;box-shadow:0 6px 20px rgba(0,0,0,0.5);">
              <i data-lucide="download" style="width:18px;height:18px;"></i> تحميل بدقة HD
            </a>
          </div>
        `;
      ...
      el.onclick = () => {
        ...
        if (previewContainer) {
          previewContainer.innerHTML = `
            ...
            <div style="position:absolute;bottom:16px;display:flex;gap:10px;">
              <a href="${item.url}" target="_blank" download="chat-ai-image.jpg" class="gradient-btn" style="padding:10px 20px;border-radius:8px;text-decoration:none;display:flex;align-items:center;gap:8px;font-size:0.95rem;box-shadow:0 6px 20px rgba(0,0,0,0.5);">
                <i data-lucide="download" style="width:18px;height:18px;"></i> تحميل بدقة HD
              </a>
            </div>
          `;
