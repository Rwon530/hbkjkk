...
class ChatAIApp {
  constructor() {
    ...
    this.activeView = 'chat'; // 'chat' | 'code' | 'article' | 'image'
  ...
  attachMessageEventListeners() {
    ...
    // Run code button (Live Code Runner)
  ...
  // --- Sidebar Chat History Filtering ---
  ...
  // --- Code Studio Logic & Live Playground ---
  initCodeStudio() {
    ...
    // Code tools buttons (Explain, Fix, Optimize, Translate)
  ...
  openPlaygroundWithCode(code, lang) {
    ...
    if (editor) {
      if (lang === 'html' || code.includes('<html') || code.includes('<div')) {
        ...
        editor.value = `<!DOCTYPE html>\n<html>\n<head>\n<style>\n${code}\n</style>\n</head>\n<body>\n  <div class="demo-box">معاينة تأثيرات CSS</div>\n</body>\n</html>`;
  ...
  initArticleStudio() {
    ...
    if (generateBtn) {
      generateBtn.onclick = async () => {
        ...
        if (outputArea) {
          outputArea.innerHTML = '<div style="text-align:center;padding:40px;color:var(--text-muted);"><i data-lucide="sparkles" class="animate-spin" style="width:32px;height:32px;margin-bottom:12px;"></i><p>جاري صياغة مقالك بأعلى درجات الاحترافية...</p></div>';
          UI.refreshIcons();
        }
        ...
        try {
          ...
          if (outputArea) {
            outputArea.innerHTML = UI.renderMarkdown(articleContent);
            UI.refreshIcons();
            UI.triggerConfetti();
          }
  ...
  initImageStudio() {
    ...
    if (enhanceBtn && promptInput) {
      enhanceBtn.onclick = async () => {
        ...
        enhanceBtn.innerHTML = '<i data-lucide="sparkles" class="animate-spin" style="width:14px;height:14px;"></i> تحسين...';
        UI.refreshIcons();
  ...
  async handleGenerateImage() {
    ...
    if (generateImgBtn) {
      ...
      generateImgBtn.innerHTML = '<i data-lucide="loader" class="animate-spin" style="width:16px;height:16px;"></i> جاري التصميم والتوليد...';
      UI.refreshIcons();
    }
    ...
    if (previewContainer) {
      previewContainer.innerHTML = `
        <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:360px;width:100%;background:rgba(0,0,0,0.2);border-radius:16px;">
          <div class="welcome-logo" style="width:60px;height:60px;margin-bottom:14px;">
  ...
  initSettingsModal() {
    ...
    if (testBtn) {
      testBtn.onclick = async () => {
        ...
        testBtn.innerHTML = '<i data-lucide="loader" class="animate-spin" style="width:16px;height:16px;"></i> فحص الاتصال...';
        UI.refreshIcons();
  ...
  populateSettingsForm() {
    ...
    if (gModel) gModel.value = this.settings.geminiModel || 'gemini-2.0-flash';
  ...
  updateApiStatusBadge() {
    ...
    const names = {
      ...
      gemini: `Gemini (${this.settings.geminiModel || '2.0'})`,
  ...
  setupEventListeners() {
    ...
    // Chat Textarea Auto Resize & Enter Submit
    const chatInput = document.getElementById('chat-input');
    if (chatInput) {
    ...
    // Provider Selector Change inside Settings
